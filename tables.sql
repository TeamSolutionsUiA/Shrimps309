DROP TABLE "ORDER";
DROP TABLE "INTERESTED_LIST";
DROP TABLE "COLLECTION_ARTWORK";
DROP TABLE "COLLECTION";
DROP TABLE "EVENT_ARTWORK";
...
CREATE TABLE "ACCOUNT" (
    ...
    CONSTRAINT ACCOUNT_EMAIL_MAILCHCK CHECK (REGEXP_LIKE (ACCOUNT_EMAIL, '^(\S+)@(\S+).(\S+)$'))
);
...
CREATE TABLE CREDIT_CARD (
    ...
    --CONSTRAINT EXPIRECHCK CHECK (CREDIT_CARD_EXPIRE_DATE BETWEEN DATE_TRUNC(SYSDATE) AND DATE_TRUNC(SYSDATE + 99*365)),
    CONSTRAINT CREDIT_CARD_NUMBER_NBRCHCK CHECK (REGEXP_LIKE(CREDIT_CARD_NUMBER, '^[[:digit:]]+$'))
);
...
CREATE TABLE ARTWORK (
    ...
    ARTWORK_SIZE_DEPTH NUMBER(7,4),
    ARTWORK_SIZE_WIDTH NUMBER(7,4),
    ARTWORK_SIZE_HEIGHT NUMBER(7,4),
    ARTIST_DISPLAY_NAME VARCHAR2(20),
    ...
    CONSTRAINT FK_ARTWORK_ARTIST FOREIGN KEY (ARTIST_DISPLAY_NAME)
        REFERENCES ARTIST (ARTIST_DISPLAY_NAME)
);
...
CREATE TABLE COLLECTION (
    ...
    COLLECTION_DESCRIPTION VARCHAR2(200) NOT NULL,
    COLLECTION_CRITERIA VARCHAR2(150),
...
CREATE TABLE INTERESTED_LIST (
    ...
    CONSTRAINT PK_INTERESTED_LIST PRIMARY KEY (ACCOUNT_EMAIL, ARTWORK_ID),
	...
	CONSTRAINT FK_INTERESTED_LIST_ACCOUNT FOREIGN KEY (ACCOUNT_EMAIL)
		REFERENCES "ACCOUNT" (ACCOUNT_EMAIL),
    CONSTRAINT INTERESTED_LIST_EMAIL_MAILCHCK CHECK (REGEXP_LIKE (INTERESTED_LIST_EMAIL, '^(\S+)@(\S+).(\S+)$'))
);
